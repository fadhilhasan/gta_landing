...
const Outro = () => {
  useEffect(() => {
    ...
    media.add(
      ...
      () => {
        ...
        const fade = gsap.quickTo(glow, "opacity", {
          duration: 0.35,
        });
        ...
        const move = (event) => {
          if (
            ...
            Number(gsap.getProperty(scene, "opacity")) < 0.95
          ) {
            hide();
            return;
          }
  ...
  useGSAP(() => {
    ...
    const updateVisibility = (self) => {
      ...
      setOverlayOpacity(
        gsap.utils.clamp(0, 1, (self.scroll() - self.start) / fadeDistance),
      );
    };
     ...
     *
     * "scale" is a GSAP alias for scaleX + scaleY.
     * Some Safari versions can throw:
     *
     * InvalidCharacterError:
     * Invalid qualified name: 'scaleX,scaleY'
     *
    ...
    const followFooter = () => {
      ...
      const artworkTop =
        center +
        (logo.getBoundingClientRect().top - currentY - center) / currentScale;

      const artworkBottom =
        center +
        (platforms.getBoundingClientRect().bottom - currentY - center) /
          currentScale;
      ...
      const availableBottom = Math.min(
        overlay.clientHeight - 24,
        footer.getBoundingClientRect().top - gap,
      );

      const scale = Math.min(
        1,
        Math.max(0.05, (availableBottom - safeTop) / artworkHeight),
      );

      const fittedBottom = center + (artworkBottom - center) * scale;

      // Safari-safe scale update
      gsap.set(fit, {
        scale,
      });
